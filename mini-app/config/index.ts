...
/** stripWxssUnsupported: 移除 WXSS 不支持的 CSS 选择器（如 :has()） */
const stripWxssUnsupported = {
  postcssPlugin: 'strip-wxss-unsupported',
  Once(root) {
    root.walkRules((rule) => {
      if (rule.selector && /:\s*has\s*\(/i.test(rule.selector)) {
        rule.remove();
        return;
      }
      if (rule.selector) {
        const cls = rule.selector.replace(/\\/g, '');
        if (cls.includes('bg-gradient') || cls.includes('from-') || cls.includes('via-') || cls.includes('to-')) {
          rule.remove();
        }
      }
    });
    root.walkDecls((decl) => {
      if (decl.prop === 'background-image' && /linear-gradient|radial-gradient|conic-gradient/i.test(decl.value)) {
        decl.remove();
      }
    });
  },
...
const generateTTProjectConfig = (outputRoot: string) => {
  const config = {
    miniprogramRoot: './',
    projectname: 'ZendMind-mini-program',
      ...
      vitePlugins: [
        {
          ...
          config(config) {
            ...
            if (typeof config.css?.postcss === 'object') {
              ...
              config.css?.postcss.plugins?.push(stripWxssUnsupported);
            }
          },
        },
        {
