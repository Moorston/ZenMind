...
- H5 上传图片偶发裂开：ZendMinde 平台 SW 拦截 blob fetch → H5 端取原始 File 对象手动构建 FormData 上传，绕过 Taro uploadFile
...
```
meditation/
  package.json          # 根级工作空间编排（dev/build/seed 脚本）
  pnpm-workspace.yaml   # 包含 mini-app + server
  mini-app/             # 前端应用（Taro 4 + React 18 + Tailwind CSS 4）
    src/                # 前端源码
    config/             # Taro 配置
    package.json        # 前端依赖（name: "mini-app"）
    tsconfig.json       # 前端 TypeScript 配置
    babel.config.js
    eslint.config.mjs
  server/               # 后端服务（NestJS 10 + Drizzle ORM + SQLite）
    src/                # 后端源码
    dist/               # 编译输出
    package.json        # 后端依赖（name: "server"）
...
```bash
pnpm dev          # 同时启动前端 H5 开发服务器 (端口 5000) 和后端 API 服务器 (端口 3000)
pnpm dev:web      # 仅启动前端
pnpm dev:server   # 仅启动后端
pnpm build        # 构建所有项目
pnpm seed         # 运行数据库种子脚本
pnpm validate     # 运行 lint + TypeScript 检查
```
